# This script runs models for R0 and 
...
mod1 = lm(log(R0_1)~Urbanization + AvgWealth + log(PopDen) + Age + log(notravel) + log(disttravel) + log1p(n_crisis_1w) + as.factor(STATE_UT),data=as.data.frame(regdat))
...
mod2 = lm(log(R0_2)~Urbanization + AvgWealth + log(PopDen) + Age + log(notravel) + log(disttravel) + log1p(n_crisis_2w) + as.factor(STATE_UT),data=as.data.frame(regdat))
...
dep.var.caption = c("log (R0)"),
column.labels = c("OLS","Spatial Lag","Spatial Error","OLS","Spatial Lag","Spatial Error"),
out = "D:/GoogleDrive/Workstation/Projects/Health/COVID-19-F/COVID-19/Figures/Script_for_paper_v2/Regression Table/R0.doc",no.space=T,single.row=T
)
...
mod1 = lm(log(Rt_1)~Urbanization + AvgWealth + log(PopDen) + Age + log(notravel) + log(disttravel) + log1p(n_crisis_1w) + as.factor(STATE_UT),data=as.data.frame(regdat))
SRmod_lag1 = lagsarlm(mod1$call$formula,data=regdat,lw,zero.policy=T)
SRmod_error1 = errorsarlm(mod1$call$formula,data=regdat,lw,zero.policy=T)

mod2 = lm(log(Rt_2)~Urbanization + AvgWealth + log(PopDen) + Age + log(notravel) + log(disttravel) + log1p(n_crisis_2w) + as.factor(STATE_UT),data=as.data.frame(regdat))
SRmod_lag2 = lagsarlm(mod2$call$formula,data=regdat,lw,zero.policy=T)
SRmod_error2 = errorsarlm(mod2$call$formula,data=regdat,lw,zero.policy=T)
...
column.labels = c("OLS","Spatial Lag","Spatial Error","OLS","Spatial Lag","Spatial Error"),
out = "D:/GoogleDrive/Workstation/Projects/Health/COVID-19-F/COVID-19/Figures/Script_for_paper_v2/Regression Table/Rt.doc",no.space=T,single.row=T
)
...
CumProp_1_moran = moran.mc(regdat$R0_1,lw,nsim=1000,zero.policy=T,na.action=na.omit)
print(CumProp_1_moran)

CumProp_2_moran = moran.mc(regdat$R0_2,lw,nsim=1000,zero.policy=T,na.action=na.omit)
print(CumProp_2_moran)

CumRate_1_moran = moran.mc(regdat$Rt_1,lw,nsim=1000,zero.policy=T,na.action=na.omit)
print(CumRate_1_moran)

CumRate_2_moran = moran.mc(regdat$Rt_2,lw,nsim=1000,zero.policy=T,na.action=na.omit)
